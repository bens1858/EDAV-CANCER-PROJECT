...
=======
PolicyIndicdenceMortality <- full_join(OperationalPolicy2021, MortalityIncidenceMerge, join_by(Country)) |>filter(Incidence_rate != is.na(Incidence_rate), Value != is.na(Value))|> dplyr::select(Incidence_rate,Mortality_rate,Country,Value,Incidence_cum_risk_74,Mortality_cum_risk_74,Continent)|>mutate(Value = ifelse(Value=="No","Operational Policy Does Not Exist", ifelse(Value=="Yes","Operational Policy Exists","No response")))
...
PolicyIndicdenceMortality$Continent[PolicyIndicdenceMortality$Country == "Micronesia"] <- "Oceania"
...
<<<<<<< HEAD
=======
ggplot(AllCancerData_lumps_new_4, aes(x=Estimate,y=fct_reorder2(Subgroup,Estimate_Level=="Low",Estimate,.desc=FALSE), color =Estimate_Level)) +
  geom_point()+
